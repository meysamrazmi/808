  ...
  $(window).scroll(function() {
      if($('#main-menu').offset().top - $(window).scrollTop() < 1) {
        $('#main-menu').css({position:"fixed", top:0, right:0, left: 0, background: '#fbfbfb'});
      }
      if ($(window).scrollTop() < 155)
        $('#main-menu').css({position:"", top:0, background: '#fbfbfb'});
    });
    ...
    window.setInterval(ads808scroll, waitTime);
    function ads808scroll(){
        var top = $('#ads808').scrollTop();
        var count = $('#ads808 .block').length;
        $('#ads808').animate({scrollTop: ((((top/250) +1)%count)*250)}, 800);
    }
});
