...
function prices() {
	$('.node-product , .node-product-kit').each(function(){
		...
		if (list_price <= display_price){
            ...
            if(!el.find('.vip-price').length && (!is_teaser || $('.role-Vip').length) && el.hasClass('apply-vip-discount')) {
    ...
    function product_teaser_prices(){
        ...
        if (list_price <= display_price){
            ...
            //there is no other discount enabled so we show vip price
			if(!el.find('.vip-price').length && $('.role-Vip').length && el.hasClass('apply-vip-discount')) {
				el.addClass('vijeh')
